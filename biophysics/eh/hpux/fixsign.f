...
c********************************************************************
     ...
     *resname(natmax),segname(natmax)
      ...
      enddo
      if(npb+nlp+nbd.ne.ndim.or.mod(npb,6).ne.0.or.mod(nbd,2).ne.0) 
     ...
     :       infosign6(2,iat,iaro),tmax0,iat=1,6)
      ...
      enddo
      if(idummy.ne.ndim.or.iaro.ne.npb) stop 'error in fixsign.f'
      close(16)
 ...
 105     if(bname(ibd,1).eq.'PB') then
     ...
     *resname(natmax),segname(natmax)
      ...
      common/iounit/iin,iout,istart(40),iungf,iunhl
      common/cring/idc6(6,10),ovar(6,6,10)
      dimension infosign(2,ndim),t(ndim,ndim)
     ...
     :         idummy,(infosign(k,inor),k=1,2),tmax0
      ...
      enddo
      if(idummy.ne.ndim.or.inor.ne.nlp+nbd) stop 'error in compsign'
      close(16)
 ...
 105     if(bname(ibd,1).ne.'PB') then
      ...
      subroutine findsign(ndim,t,signname)  

      implicit real*8 (a-h,o-z)
      parameter(natmax=200,ndmax=500)
      character segname*4,resname*3,bname*2,ac*4,symbol*2,signname*5
      integer ul
      common/ato/ac(natmax),symbol(40)
      common/atom2/exps(40),exps2(40),expp(40),expp2(40),expd(40),              
     *expd2(40),expf(40),expf2(40),cs1(40),cs2(40),cp1(40),cp2(40),             
     ...
     *resname(natmax),segname(natmax)
      ...
      common/iounit/iin,iout,istart(40),iungf,iunhl
      common/cring/idc6(6,10),ovar(6,6,10)
      dimension infosign(2,ndim),infosign6(2,6,10)
      dimension tmax6(6)
      dimension t(ndim,ndim)

      open(16,file=signname)
 ...
 105     if(bname(ibd,1).ne.'PB') then
	    ...
	    tmax=0.0
	    jmax=0
	    do j = ll(labar(ibd,iat+1)),ul(labar(ibd,iat+1))
	       if(dabs(t(j,i)).gt.tmax) then
	          tmax=dabs(t(j,i))
	          jmax=j
	       endif
