...
c        converter2 = 12000.d0/1.6d0/6.022d0
...
c        sum2=0.d0
c        do k = 1 , namb
c          sum2=sum2+rl(k)*rl(k)
c          sum1=sum1+2.d0*vadd(k)*converter*rl(k)*rl(k)
c     :        -amassLam(k)*vl(k)*vl(k)
c        enddo
c        gamma=sum1/sum2*2.d0

        do k = 1 , namb
           fl(k)=-2.d0*rl(k)*vadd(k)*converter
           fl(k)=fl(k)+amassLam(k)*(
     :           vx(nsolute)*vx(nsolute)+
     :           vy(nsolute)*vy(nsolute)+
     :           vz(nsolute)*vz(nsolute))*rl(k)+
     :           gamma*rl(k)
        enddo
