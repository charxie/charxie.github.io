...
c  *** in input file ipr must be set > 3 as to get dm          
     ...
     :rx(nazr),ry(nazr),rz(nazr)
      ...
      do ibd = 1 , n
         ...
         if(symbol(label(ibd,4))(2:2).eq.'O'
     :      .and.label(ibd,1).eq.'LP') then
      ...
      do i = 1 , num_lonepair, 2
         ...
         if(label(ind1_lonepair(i),4).ne.label(ind1_lonepair(i+1),4))
...
c	 write(6,'(2f10.5)') ((v(m,k),k=1,2),m=1,2)
     ...
     :rx(nazr),ry(nazr),rz(nazr)
      ...
      do ibd = 1 , n
         ...
         if((symbol(label(ibd,4))(2:3).eq.'OD'
     :      .or.symbol(label(ibd,4))(2:3).eq.'OG')
     :      .and.label(ibd,1).eq.'LP') then
