...
c  *** in input file ipr must be set > 3 as to get dm          
     ...
     $NELECS,NOCCA,NOCCB
      ...
      COMMON/EXTRA/TNEW(NBSZR,NBSZR),IHYB(NAZR),TOPO(NAZR,NAZR),
     $T2J(NBSZR)
...
c      write(6,'(100i3)') (ibx(i),i=1,n)
c      write(6,'(100i3)') (label(i,4),i=1,n)
c      write(6,'(100i3)') (label(i,5),i=1,n)

      do ibd = 1 , n
      
         if((symbol(label(ibd,4))(2:2).eq.'C'
     : .and. symbol(label(ibd,5))(2:2).eq.'O')
     : .or. (symbol(label(ibd,4))(2:2).eq.'O'
     : .and. symbol(label(ibd,5))(2:2).eq.'C')
     :     ) then
     
            if(symbol(label(ibd,4))(2:3).eq.'OG'.or.
     :         symbol(label(ibd,5))(2:3).eq.'OG'.or.
     :         symbol(label(ibd,4))(2:3).eq.'OH'.or.
     :         symbol(label(ibd,5))(2:3).eq.'OH'.or.
     :         symbol(label(ibd,4))(2:3).eq.'OD'.or.
     :         symbol(label(ibd,5))(2:3).eq.'OD'
     :        ) goto 2020            
 ...
 2020       continue
      ...
      do i = 1 , num_cobond, 2
         ...
         if(label(ind1_cobond(i),4).ne.label(ind1_cobond(i+1),4)
     :  .or.label(ind1_cobond(i),5).ne.label(ind1_cobond(i+1),5)) 
