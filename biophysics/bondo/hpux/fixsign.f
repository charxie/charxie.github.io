...
c is created and compared with the signs stored in the comparison array.
     ...
     :rx(nazr),ry(nazr),rz(nazr)
      ...
      nbd=0
      do i = 1 , n
         if(label(i,1).eq.'PB') npb=npb+1
         if(label(i,1).eq.'LP') nlp=nlp+1
         if(label(i,1).eq.'BD') nbd=nbd+1      
      enddo
	   ...
	   do iat = 1 , 6
	      if(infosign6(1,iat,iaro).ne.0) then
	         tmax6=t(infosign6(1,iat,iaro),i)
                 ...
                 if(itemp.eq.-1) then
                    do j = ll(idc6(iat,label(ibd,3))),
     :                     ul(idc6(iat,label(ibd,3)))
                       t(j,i)=-t(j,i)
                    enddo
                 endif
              endif
...
c      enddo
 ...
 105     if(label(ibd,1).ne.'PB') then
	    ...
	    tmax=0.0
	    jmax=0
	    do j = ll(idc6(iat,label(ibd,3))),ul(idc6(iat,label(ibd,3)))
	       if(dabs(t(j,i)).gt.tmax) then
	          tmax=dabs(t(j,i))
	          jmax=j
	       endif
	    enddo
	    infosign6(1,iat,label(ibd,3))=jmax
	    if(t(jmax,i).ge.0.d0) then
	       infosign6(2,iat,label(ibd,3))= 1
	    else
	       infosign6(2,iat,label(ibd,3))=-1
	    endif
	    tmax6(iat)=tmax
	 ...
	 if(label(ibd,1).eq.'LP')
     :      write(16,'(i10,2x,a2,a1,2x,i5,a2,5x,2x,2i5,f8.4)') 
     :       i,label(ibd,1),label(ibd,2),
     :       label(ibd,4),symbol(label(ibd,4))(1:2),
     :       (infosign(k,i),k=1,2),tmax
	 if(label(ibd,1).eq.'BD')
     :      write(16,'(i10,2x,a2,a1,2x,i5,a2,i5,a2,2i5,f8.4)') 
     :       i,label(ibd,1),label(ibd,2),
     :       label(ibd,4),symbol(label(ibd,4))(1:2),
     :       label(ibd,5),symbol(label(ibd,5))(1:2),
     :       (infosign(k,i),k=1,2),tmax
	 if(label(ibd,1).eq.'PB')
     :      write(16,'(i10,2x,a2,a1,i5,2x,6(i5,a2,2i5,f8.4))') 
     :       i,label(ibd,1),label(ibd,2),label(ibd,4),
     :       (idc6(ia,label(ibd,3)),symbol(idc6(ia,label(ibd,3)))(1:2),
     :       infosign6(1,ia,label(ibd,3)),
     :       infosign6(2,ia,label(ibd,3)),tmax6(ia),
     :       ia=1,6)
      enddo
